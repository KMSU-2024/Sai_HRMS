page 33066136 "Sanctioned v/s Vacant"
{
    ...
    actions
    {
        area(Processing)
        {
            action(Update)
            {
                ...
                trigger OnAction()var DocumentAttachment: Record "Document Attachment";
                begin
                    isUpdateMode:=true;
                    UpdateMode:=true;
                    ATtachB:=true;
            ...
            action(SubmitToHistory)
            {
                ...
                trigger OnAction()var SanctionedHistory: Record "Sanctioned Post History";
                ...
                begin
                    ...
                    if SanctionedHistory.FindLast()then if SanctionedHistory."New Sanctioned Strength" = Rec."Sactioned Strength" then Error('This sanctioned strength has not been updated.');
                    DocumentAttachment.SetRange("Table ID", Database::"Sanctioned v/s Vacant");
                    DocumentAttachment.SetRange("No.", Rec."Sanctioned Post No.");
            ...
            action(SanctionedPostHistory)
            {
                ...
                trigger OnAction()var SanctionedHistoryList: Record "Sanctioned Post History";
                begin
    ...
    trigger OnNewRecord(BelowxRec: Boolean)var NoSeriesMgt: Codeunit NoSeriesManagement;
    begin
        IsNewRecord:=true;
        ATtachB:=true;
        EditBolean:=true;
    ...
    trigger OnQueryClosePage(CloseAction: Action): Boolean var ConfirmResult: Boolean;
    ChangesMade: Boolean;
    begin
        RevertChanges:=false; // Reset the flag
        ChangesMade:=(Rec."Department/Trade/Section" <> OriginalRec."Department/Trade/Section") or (Rec.Designation <> OriginalRec.Designation) or (Rec."Sactioned Strength" <> OriginalRec."Sactioned Strength") or (Rec."Approval No." <> OriginalRec."Approval No.") or (Rec."Financial Year" <> OriginalRec."Financial Year") or (Rec.Remarks <> OriginalRec.Remarks);
        ...
        if(ChangesMade or IsNewRecord) and not Rec.submit then begin
            ...
            if ConfirmResult and ChangesMade and not IsNewRecord then begin
                ...
                RevertChanges:=true;
                if(Rec."XSactioned Strength" <> 0)then rec."Sactioned Strength":=Rec."XSactioned Strength";
                if rec."XApproval No." <> '' then Rec."Approval No.":=rec."XApproval No.";
                if rec.xRemarks <> '' then rec.Remarks:=rec.xRemarks;
                if XFinancialYear <> '' then rec."Financial Year":=XFinancialYear;
                rec.Modify(TRUE);
                exit(true); // Allow the page to close, OnClosePage will handle the revert/delete
            end
            else if ConfirmResult and IsNewRecord then rec.Delete()
                else
                begin
  ...
  */
    // ... (other parts of the page remain the same) ...
    local procedure ChangesExist(): Boolean begin
        exit((Rec."Department/Trade/Section" <> OriginalRec."Department/Trade/Section") or (Rec.Designation <> OriginalRec.Designation) or (Rec."Sactioned Strength" <> OriginalRec."Sactioned Strength") or (Rec."Approval No." <> OriginalRec."Approval No.") or (Rec."Financial Year" <> OriginalRec."Financial Year") or (Rec.Remarks <> OriginalRec.Remarks));
    end;
    local procedure GetLastStrength(): Integer var SanctionedHistory: Record "Sanctioned Post History";
    begin
        SanctionedHistory.SetRange("Sanctioned Post No.", Rec."Sanctioned Post No.");
        if SanctionedHistory.FindLast()then exit(SanctionedHistory."New Sanctioned Strength");
        exit(0);
    end;
    local procedure HasHistoryRecord(): Boolean var SanctionedHistory: Record "Sanctioned Post History";
    begin
        SanctionedHistory.SetRange("Sanctioned Post No.", Rec."Sanctioned Post No.");
