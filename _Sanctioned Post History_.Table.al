table 33066429 "Sanctioned Post History"
{
    ...
    trigger OnInsert()var SanctionedPostHistory: Record "Sanctioned Post History";
    begin
        SanctionedPostHistory.Reset();
        if SanctionedPostHistory.FindLast()then Rec.ApplicantNo:=SanctionedPostHistory.ApplicantNo + 1
        else
            Rec.ApplicantNo:=1;
    end;
}
