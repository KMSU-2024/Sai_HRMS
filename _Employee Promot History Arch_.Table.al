table 33066433 "Employee Promot History Arch" //SS07OCT c
{
    ...
    procedure GetNextVersion(EmployeeNo: Code[20]): Integer var EmpArchive: Record "Employee promot history Arch";
    begin
        EmpArchive.Reset();
        EmpArchive.SetRange("HRMS ID", EmployeeNo);
        if EmpArchive.FindLast()then exit(EmpArchive."Version No." + 1)
        else
            exit(1);
    end;
}
