 ...
 */
    procedure ArchiveEmployee(var EmployeeRec: Record Employee)var EmployeeArchive: Record "Employee History";
    LastVersion: Integer;
    begin
        EmployeeArchive.Reset();
        EmployeeArchive.SetRange("Employee No.", EmployeeRec."No."); // ✅ Filter by Employee No.
        if EmployeeArchive.FindLast()then LastVersion:=EmployeeArchive."Version No."
        else
            LastVersion:=0;
        EmployeeArchive.Init();
        EmployeeArchive.Validate("Employee No.", EmployeeRec."No.");
    ...
    var FileMngCU: Codeunit "File Management";
    InStr: InStream;
    ...
    var FinUpgIncr: Record "Financial Upg Application";
    begin
    ...
    var hist: Record "Financial Upg App history";
    ...
    procedure ArchiveFinUpgApp(var Rec: Record "Financial Upg Application")var ArchiveFinUpgTab: Record "Financial Upg App history";
    entrno: Integer;
    begin
  ...
  */
    procedure ArchiveEmployeeTransfer(var Rec: Record "Transfer Joining History")var ArchiveRec: Record "Archive Employee Transfer";
    VersionNo: Integer;
    begin
        // Get last version for this employee
        ArchiveRec.Reset();
        ArchiveRec.SetRange("HRMS ID", Rec."HRMS ID");
        if ArchiveRec.FindLast()then VersionNo:=ArchiveRec.Slno + 1
        else
            VersionNo:=1;
        ArchiveRec.Init();
