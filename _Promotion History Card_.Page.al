page 33065740 "Promotion History Card"
{
    ...
    layout
    {
        area(Content)
        {
            group(GroupName)
            {
                ...
                field("Promotion Order File Name"; rec."Promotion Order File Name")
                {
                    ApplicationArea = All;
                }
            }
    ...
    actions
    {
        area(Processing)
        {
            action("Submit Promotion1") //SS07OCT
            {
                ...
                trigger OnAction()
                var
                    PromoHistArchive: Record "Employee Promot History Arch";
                ...
                begin
                    ...
                    if PromoHistArchive.FindLast() then
                        LastVersion := PromoHistArchive."Version No."
                    else
                        LastVersion := 0;
                    ...
                    if PROMTOIONARCH.FindLast() then
                        rec."From Designation" := PROMTOIONARCH."To Designation"
                    else
    ...
    //sai start
    local procedure HasCriticalChanges(): Boolean
    var
        OriginalRec: Record "Employee Promotion History";
    begin
    ...
    trigger OnOpenPage()
    var
        LoadedRec: Record "Employee Promotion History"; //SS07OCT
    begin
        if Rec."HRMS ID" <> '' then
            iseditable := false
