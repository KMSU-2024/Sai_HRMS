page 33065731 "Employee Transfer Relief Card"
{
        ...
        area(Processing)
        {
            ...
            group("Transfer Order Document")
            {
                action(UploadTransferOrder) //SS07OCT
                {
                    ...
                    trigger OnAction()var FileMgt: Codeunit "File Management";
                    LocalFileName: Text;
                    begin
 ...
 '.pdf;.jpg' // extension filter
                ...
                action(DownloadTransferOrder1) //SS07OCT
                {
                    ...
                    trigger OnAction()var InS: InStream;
                    FileName: Text;
                    begin
                        if TempTransferDoc.HasValue then begin
                            TempTransferDoc.CreateInStream(InS);
                            if UploadedFileName <> '' then FileName:=UploadedFileName
                            else
    ...
    var var TempTransferDoc: Codeunit "Temp Blob"; // holds uploaded file temporarily //SS07OCT
