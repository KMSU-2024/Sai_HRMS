codeunit 33065928 AttachmentsCodeunit //creating attachments and copying attachments
{
    ...
    begin
        case DocumentAttachment."Table ID" of
            DATABASE::"Sanctioned v/s Vacant":
                begin
                    RecRef.Open(DATABASE::"Sanctioned v/s Vacant");
                    if sanctionedPost.Get(DocumentAttachment."No.") then
                        RecRef.GetTable(sanctionedPost);
                end;
            DATABASE::"Sanctioned Post History":
                begin
    ...
    procedure CopyAttachments(OrderConfirmation: Record "Sanctioned v/s Vacant")
    var
    ...
    begin
        ...
        if not PostedOrderConfirmation.FindFirst() then
            Error('Posted Order for %1 not found.', OrderConfirmation."Sanctioned Post No.");
        // Get next ID for attachments
        DocumentAttachment.Reset();
        DocumentAttachment.SetRange("Table ID", Database::"Sanctioned Post History");
        if DocumentAttachment.FindLast() then
            DCID := DocumentAttachment.ID + 1
        else
            DCID := 1;
        // Copy attachments
        DocumentAttachment.Reset();
    ...
    procedure DownloadSanctionedPostDocument(No: CODE[20]): Text
    var
        SancPost: Record "Sanctioned v/s Vacant";
    begin
        SancPost.Get(No);
        exit(SancPost."File Name");
    end;

    procedure SubmitHistorySanctionPost(No: Code[20]; ApprovalNo: Code[20]; FinYear: Code[20]; Remarks: Text[60]; SanctionStrength: Integer) //megha 30-10-25
    var
    ...
    begin
        ...
        if SanctionedHistory.FindLast() then
            if SanctionedHistory."New Sanctioned Strength" = SanctPost."Sactioned Strength" then
    ...
    //var
    //pg: Page 60525;
    procedure UpdateSanctPost(No: Code[20])
